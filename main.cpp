...
#define PRINT_WORNING(message) printf("\nworning %s(%d) IN %s: %s\n", LOCATION, message);
...
namespace stck {
    ...
    struct Secure_Stack_t {
        ...
        unsigned size;      //Хранит длину стека в type_Stack
        unsigned number;
        unsigned real_size; //В байтах
        int Sec_Level;
...
int Dump (stck::Secure_Stack_t *Stack);

inline void set_canary_to_Stack (stck::Secure_Stack_t *Stack);

int CheckingCanary (stck::Secure_Stack_t *Stack, bool write = false);
...
int main () {
    ...
    My_Stack.size = size;
    My_Stack.Sec_Level = 1;
    My_Stack.expansion_coef = 0.2;

    StackConstruct (&My_Stack);

    for (int i = 0; i < size; i++)
        StackPush (&My_Stack, 17 + i);
    stck::type_Stack temp = 0;
    int state = 0;

    for (int i = 0; !StackPop (&My_Stack, &temp); i++)
        printf ("%d) %d\n", i, temp);
...
int StackPush (stck::Secure_Stack_t *Stack, stck::type_Stack value) {
    ...
    if (Stack->number >= Stack->size) { //Расширяем стек
        Stack->size = (unsigned) ((float) Stack->size * (1 + Stack->expansion_coef));
        unsigned size_stack = Stack->size * sizeof (stck::type_Stack);
        switch (Stack->Sec_Level) {
            case 2:;
            case 1:
                size_stack += 2 * sizeof (stck::canary_t);
        }
        Stack->begin = (stck::type_Stack *) realloc (Stack->begin, size_stack);
...
//----------------------------------------------------------------------
int StackConstruct (stck::Secure_Stack_t *Stack) {
    if (Stack->size <= (unsigned) Stack->expansion_coef + 1) {   // Проверка на мин. размер стека, при котором работает
        Stack->size = (unsigned) Stack->expansion_coef + 1;      // линейное расширение в 1+expansion_coef раз
    }
    unsigned size_stack = Stack->size * sizeof (stck::type_Stack); //Хранит кол-во байт
    ...
    switch (Stack->Sec_Level) {
        ...
        case 0: {
            ...
            Stack->number = 0;
            Stack->real_size = size_stack;
            switch (Stack->Sec_Level) { // Дополнительные действия для модулей безопасности
                case 1: {
                    set_canary_to_Stack (Stack);
